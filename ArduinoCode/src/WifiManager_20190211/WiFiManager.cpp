...
bool WiFiManager::wifiConnectNew(String ssid, String pass){
  ...
  DEBUG_WM(DEBUG_DEV,F("Using Password:"),pass);
   // do Connection callback if set #m.nu
  if ( _connectcallback != NULL) { //#m.nu
    _connectcallback(ssid); //#m.nu
  } //#m.nu
...
bool WiFiManager::wifiConnectDefault(){
  ...
  DEBUG_WM(DEBUG_DEV,F("Using Password:"),WiFi.psk());
   // do Connection callback if set #m.nu
  if ( _connectcallback != NULL) { //#m.nu
    _connectcallback(WiFi_SSID()); //#m.nu
  } //#m.nu
...
void WiFiManager::handleRoot() {
  ...
  page += FPSTR(HTTP_PORTAL_OPTIONS);
   page += getInfoData("chipid");    // #m.nu PIN på förstasidan på config panelen
  page += getMenuOut();
  reportStatus(page);
  page += FPSTR(HTTP_END);
...
void WiFiManager::handleInfo() {
  ...
  //@todo convert to enum or refactor to strings
    ...
    String infoids[] = {
      ...
      F("uptime"),
      F("chipid"),
	     /*  F("fchipid"),   //#m.nu kommenterar bort data som inte ska visas på info sidan på config panelen
      F("idesize"),
      F("flashsize"),
      F("sdkver"),
      F("corever"),
...
String WiFiManager::getInfoData(String id){
  ...
  if(id==F("esphead"))p = FPSTR(HTTP_INFO_esphead);
    ...
    //p.replace(FPSTR(T_1),String(WIFI_getChipId(),HEX));
	 p.replace(FPSTR(T_1),(String)WIFI_getChipId()); //m.nu decimalt
  }
  #ifdef ESP32
...
void WiFiManager::setConnectCallback( void (*func)(String ssid) ) { //#m.nu
  _connectcallback = func;  //#m.nu
} //#m.nu
