...
#include <DNSServer.h>
#include <memory>
#include "custom_strings_swe.h" // #m.nu
...
class WiFiManager
{
  public:
    ...
    void          setAPCallback( std::function<void(WiFiManager*)> func );
	  //called when ESP tries to connect to saved WiFi #m.nu
    void          setConnectCallback( void (*func)(String ssid) ); //#m.nu
  ...
  private:
	...
	std::function<void(String)> _connectcallback; //#m.nu
